...
exports.generateCompletionCertificate = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    try {
      ...
      const finalExamPassed = userData.finalExamPassed === true;
      const finalExamScore = userData.finalExamScore || 0;
      const MIN_INSTRUCTION_MINUTES = 1440;
      const MIN_EXAM_SCORE = 75;

      if (totalInstructionMinutes < MIN_INSTRUCTION_MINUTES) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          `Insufficient instruction time. Required: ${MIN_INSTRUCTION_MINUTES} minutes, Current: ${totalInstructionMinutes} minutes`
        );
      }
      ...
      if (!finalExamPassed) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          'Final exam must be passed (75% minimum score) to earn completion certificate'
        );
      }

      if (finalExamScore < MIN_EXAM_SCORE) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          `Exam score below passing threshold. Required: ${MIN_EXAM_SCORE}%, Current: ${finalExamScore}%`
        );
      }
      ...
      if (!existingCertQuery.empty) {
        logger.log(`Completion certificate already exists for user ${userId}, course ${courseId}`);
        const existingCert = existingCertQuery.docs[0];
        return {
          certificateId: existingCert.id,
          success: true,
      ...
      const certificateNumber = `COMP-${new Date().getFullYear()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
      const awardedDate = new Date();
      ...
      await db
        ...
        .add({
          userId,
          courseId,
          eventType: 'COMPLETION_CERTIFICATE_GENERATED',
          certificateId: certificateRef.id,
          certificateNumber,
          totalInstructionMinutes,
          finalExamScore,
      ...
      throw new functions.https.HttpsError(
        'internal',
        'Failed to generate completion certificate'
      );
    }
  }
);

exports.checkCompletionCertificateEligibility = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    try {
      ...
      const finalExamPassed = userData.finalExamPassed === true;
      const finalExamScore = userData.finalExamScore || 0;
      const certificateGenerated = userData.completionCertificateGenerated === true;
      const MIN_INSTRUCTION_MINUTES = 1440;
      const MIN_EXAM_SCORE = 75;

      return {
        eligible: totalInstructionMinutes >= MIN_INSTRUCTION_MINUTES && finalExamPassed && finalExamScore >= MIN_EXAM_SCORE,
        certificateGenerated,
        totalInstructionMinutes,
        finalExamPassed,
        finalExamScore,
        minutesRemaining: Math.max(0, MIN_INSTRUCTION_MINUTES - totalInstructionMinutes),
        missingRequirements: [
          ...(totalInstructionMinutes < MIN_INSTRUCTION_MINUTES ? [`Minimum ${MIN_INSTRUCTION_MINUTES} minutes required (${totalInstructionMinutes} completed)`] : []),
          ...(!finalExamPassed ? ['Final exam must be passed'] : []),
          ...(finalExamPassed && finalExamScore < MIN_EXAM_SCORE ? [`Exam score must be ${MIN_EXAM_SCORE}% or higher (${finalExamScore}% current)`] : [])
        ]
      };
    } catch (error) {
      logger.error('Error checking completion certificate eligibility:', error);
